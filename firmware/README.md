...
This repository pairs firmware for the Nordic nRF52 + Decawave DW1000 platform with a Python based data-processing and learning pipeline. The C firmware fixes long-standing TDMA and channel impulse response (CIR) issues so a single tag can range sequentially against up to four anchors. The Python workflow cleans up the raw logs that the firmware prints, applies quality-aware filters, and trains the `UWB_HyperTuned_Model` to estimate position error and line-of-sight (LOS/NLOS) class labels.
...
| `proposed_model.py` | End-to-end Python pipeline: filtering, safe dataset split, PyTorch model, multitask loss, training, evaluation, and model persistence. |
...
## Python data pipeline (`proposed_model.py`)

### Capabilities

- Loads LOS, static NLOS, and dynamic NLOS Excel logs from `./uwb_data/los.xlsx`, `./uwb_data/nlos_static.xlsx`, and `./uwb_data/nlos_dynamic.xlsx`.
- Applies the quality-aware combo filter (SNR weighted smoothing, quality masking, and Z-score outlier replacement) or falls back to the SNR-only filter.
- Performs safe chronological splits per `point_id` and `nlos_type` so the validation and test sets never leak future measurements of the same point.
- Builds anchor-local sliding windows and scales them with `StandardScaler`.
- Trains the `UWB_HyperTuned_Model`, a shared-attention multitask network that predicts:
  - Position error regression (cm)
  - LOS/NLOS class labels (3 classes)
  - Confidence score tied to the regression residual
- Uses `UWBMultiTaskLoss` to balance regression, classification, and confidence supervision with optional class weights inferred from the label encoder.
- Saves artifacts under `saved_models/` (`*.pth`, scaler pickle, label encoder pickle, and `model_info.json` summarizing hyperparameters).
...
pip install pandas numpy torch scikit-learn matplotlib seaborn scipy openpyxl
```

Place the Excel files in `uwb_data/` as noted above. Each sheet should include at least `point_id`, `ID` (anchor), `DIST`, `SNR`, `CIR`, and optionally `timestamp` or `group` for ordering.
...
The script prints dataset sizes, class distribution, training progress (loss, validation metrics), and final KPIs such as mean position error, P90 error, and classification accuracy. When training finishes, the best checkpoint plus scaler/label-encoder artifacts are written to `saved_models/` and `best_uwb_model.pth`.

To reuse a trained model from another script:

```python
from proposed_model import load_model
model, scaler, label_encoder = load_model('best_uwb_model.pth')
```

Feed preprocessed windows to `model(X_batch)` to obtain `(position, class_logits, confidence, attention_weights)`.
...
CSV parsers can key off `DATA,MEAS` and `DATA,RESULT`. The ML pipeline expects these columns (or derivatives) to be present in the Excel files produced from these logs.
...
- The ML section depends on PyTorch, scikit-learn, pandas, NumPy, Matplotlib, and SciPy; consult each project's license for redistribution terms.
